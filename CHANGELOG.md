...
### Changed
- Docs: o3de-mcp install instructions offer PyPI (`pip install o3de-mcp`, 0.4.0 or later) alongside the editable checkout and `PYTHONPATH` routes; the example runner's import error lists the same options.
...
- `Examples/TwinStickShooter/run_batched.py` drives the example through o3de-mcp's `begin_session` / `exec_in_session` / `end_session` tools instead of the private `_pool` transport. The gem scripts path is set up and `steps` is imported once per session; each step then runs in that persistent namespace. Requires o3de-mcp with the `mcp` 2.x SDK.
...
### Fixed
- `spawn_prefab` could crash the editor. `PrefabPublicRequestBus.InstantiatePrefab` segfaults when the template cannot be loaded (`PrefabPublicHandler::InstantiatePrefab` hands an empty DOM to `PrefabDomUtils::GetTemplateSourcePaths`, observed on O3DE 26.10.0), and a C++ segfault cannot be caught from Python. The function now rejects path-like names, locates the `.prefab` file on disk first, and returns a `prefab_not_found` error without calling the bus. It also passes an empty `EntityId()` parent instead of `None`, checks the returned outcome, and reports the spawned entity id.
- Windows compilation. Replaced `std::getenv`, `std::fopen`/`std::fprintf`, and `::mkdir` with `AZ::Utils::GetEnv`, `std::ofstream`, and a platform mkdir macro (`_mkdir` on Windows, `::mkdir` on Linux) to clear MSVC CRT warnings; switched platform guards from `AZ_TRAIT_OS_PLATFORM_WINDOWS` to `defined(AZ_PLATFORM_WINDOWS)`; added `3rdParty::OpenSSL` as a build dependency so `openssl/err.h` resolves. The gem now builds and runs on Windows with O3DE 2.7.0.
- `execute_python` result retrieval. The previous two-step flow (execute script, then retrieve the result in a second `ExecuteByString` call) relied on `sys` state persisting between calls, which does not hold, so results were lost. Execution, stdout/stderr capture, and result-file writing are now combined into a single script passed to `ExecuteByString`. The result file moved from the app root (`GetDefaultAppRootPath`, which resolves to a nonexistent `/tmp` on Windows) to `<project>/user/temp/`, and is removed on every path after reading.
...
- `twin_stick_movement.lua` used `InputEventNotificationBus.GetCurrentBusId():GetName()` to dispatch between its four bound input events. `InputEventNotificationId` has no `:GetName()` in O3DE 26050; the input handlers silently no-op'd and the Player wouldn't move. Migrated to comparing the bus id directly against stashed `InputEventNotificationId` instances via the reflected `Equal` operator (Lua `==`). Aim now uses `RigidBodyRequestBus.SetAngularVelocity` instead of `TransformBus.SetWorldRotationQuaternion`, since direct transform writes on a dynamic rigid body get overwritten by PhysX each tick (the editor warns: "Transform of Entity 'Player' with simulated body was set manually"). Note: `mouse_delta_x`/`mouse_delta_y` channels don't currently dispatch when running under the editor's Play mode because the cursor isn't captured/locked; use a standalone game launcher build to test mouse aim, or remap aim to sustained inputs.
